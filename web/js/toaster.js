...
function Toaster(
  ...
  if (duration > 0) {
    $("#" + randId)
      ...
      .queue(function () {
        rid = "#" + $(this)[0].id;
        animateCSS(rid, "fadeOut", false).then(() => {
          $(this).remove();
        });
      });
  }
  ...
  timestamp =
    '<span class="bg-gray-100 text-gray-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded mr-2 dark:bg-gray-700 dark:text-gray-400 border border-gray-500"><span class="material-symbols-rounded mr-2">timer</span>' +
    padU(now.getDate()) +
    "." +
    padU(now.getMonth() + 1) +
    " " +
...
function getIconCode(icon) {
  if (icon == "success") {
    ...
    return '<div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-gray-500 bg-gray-100 rounded-lg dark:text-gray-300 dark:bg-gray-700"><span class="material-symbols-rounded mr-2">timer</span><span class="sr-only">Clock icon</span> </div>';
  } else {
    return icon;
  }
}
